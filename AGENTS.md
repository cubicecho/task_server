...
| Choice | Why |
| --- | --- |
| **`node:sqlite` + Drizzle** | No native dependency to build, so the container is a `node:26-slim` with nothing compiled in it. The postgres swap is confined to `server/db/client.ts` |
...
hand-write a type that codegen produces, and never edit `src/gql/graphql.ts` — biome ignores
it because it is output.
