 ...
 * being turned away has a perfectly healthy last run and a pile of firings it dropped.
 */
export const HEALTH = ["refused", "broken", "running", "off", "manual", "fine"] as const;
 ...
 *
 * `running` and `broken` never compete: both read the newest non-skipped run, so a task that is
 * running now cannot also be showing the error before it.
 */
